...
// Narrowed via the truthy check, not a `!` assertion (the lint config forbids those): inside this
// branch bypassSecret is provably a string, since the guard above already ruled out the alternative.
const remoteHeaders=remoteBaseUrl&&bypassSecret?{extraHTTPHeaders:{'x-vercel-protection-bypass':bypassSecret,'x-vercel-set-bypass-cookie':'true'}}:{}
export default defineConfig({testDir:'./tests/e2e',use:{baseURL:remoteBaseUrl||'http://127.0.0.1:5174',trace:'retain-on-failure',...remoteHeaders},webServer:remoteBaseUrl?undefined:{command:'npm run preview -- --host 127.0.0.1 --port 5174 --configLoader runner',url:'http://127.0.0.1:5174/login',reuseExistingServer:true},projects:[{name:'desktop',use:{...devices['Desktop Chrome']}},{name:'mobile',use:{...devices['Pixel 7']}}]})
