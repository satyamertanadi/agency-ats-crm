...
| Interview Intelligence (Release A0) | Implemented / calibration + provider gate | Consent-gated manual transcript import (TXT/VTT/SRT/JSON), speaker mapping, versioned job interview blueprints with staleness, asynchronous evidence-backed analysis producing independent candidate-fit and consultant-quality assessments, deterministic speaking metrics, bounded transcript paging, retention and consent-withdrawal purge. Disabled for every workspace by default. Live analysis needs Anthropic credentials; the A0 end-to-end flow and calibration against 20-30 real interviews remain external. See [docs/interview-intelligence.md](interview-intelligence.md). Releases A1, A2, B1 and B2 are not built. |
