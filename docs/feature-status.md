...
| Interview Intelligence | Design contract only | No schema, no code, no behaviour. [docs/interview-intelligence.md](interview-intelligence.md) records the verified baseline, the entity mapping, and five points where the implementation plan and this repository disagree -- including that the durable job queue the asynchronous analysis needs was dropped as unreachable and would have to be reinstated. Consent policy is in [docs/interview-transcript-consent.md](interview-transcript-consent.md). Nothing here is usable until Release A0 lands. |
