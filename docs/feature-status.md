...
| Interview Intelligence (Releases A0, A1, A2, B1, B2) | Implemented / calibration + provider gate | Consent-gated manual transcript import (TXT/VTT/SRT/JSON), speaker mapping, versioned job interview blueprints with staleness, asynchronous evidence-backed analysis producing independent candidate-fit and consultant-quality assessments, deterministic speaking metrics, bounded transcript paging, retention and consent-withdrawal purge. Disabled for every workspace by default. Live analysis needs Anthropic credentials; the A0 end-to-end flow and calibration against 20-30 real interviews remain external. See [docs/interview-intelligence.md](interview-intelligence.md). A1 adds review, coaching actions and the attention list. B1 collects Google Meet transcripts automatically for workspaces that opt in, under the same consent gate and through the same ingestion function as a pasted transcript; automatic *analysis* is a second switch that stays off until calibration is accepted. A2 adds an Interview quality view on the existing Scorecard route: band distribution, dimension trends against the consultant’s own previous period, speaking share, and desk-level coverage and coaching themes, every count opening the interviews behind it. Averages are withheld below three analysed interviews, and the team aggregate carries no member identifier, so it cannot be turned into a ranking. B2 adds the daily owner brief: disabled by default, sent at a workspace-local time to members an owner names, containing counts and fixed vocabulary only -- no transcript text, candidate details or model-authored sentences -- with the same content stored in-app. All five releases are code complete; calibration against real interviews remains the gate. |
