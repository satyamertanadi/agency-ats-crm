...
| Interview transcription and AI notes | Implemented / Workspace gate | Reads the transcript Google Meet produces for a scheduled interview through the Meet REST API using the organizer's existing token; no audio is recorded or stored, and no third-party recorder joins the call. One provider call produces the summary, an evidence-backed candidate assessment quoting the transcript, and a fixed-rubric review of the consultant's interviewing, which is readable only with `interview_coaching.read`. Notes stay a draft until a consultant accepts them. Requires Google Workspace Business Standard or above with transcription enabled, the `meetings.space.readonly` scope on the OAuth client, and one reconnect per already-connected member. Transcript language coverage must be confirmed against Meet before pilot use. |
