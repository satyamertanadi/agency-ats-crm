...
Set `AI_PROVIDER=anthropic`, an explicit current `AI_MODEL`, `ANTHROPIC_API_KEY`, and a random `WORKER_SECRET` as Supabase Function secrets. Optionally set `AI_MODEL_PARSE` to a cheaper model (e.g. `claude-haiku-4-5`) for the mechanical CV and LinkedIn parsers; it falls back to `AI_MODEL` when unset, and evaluation (`generate-candidate-profile`) always uses `AI_MODEL`. Deploy `parse-candidate-cv`, then configure the GitHub secret `CV_PARSE_WORKER_SECRET` so the hourly cleanup workflow can purge unconfirmed CV drafts after 24 hours. Its value must equal the Function `WORKER_SECRET` exactly; the workflow sends it as `x-worker-secret` and `cleanup()` compares the two, so a mismatch is a 401 rather than a partial run. The URL side is already covered by the existing `PRODUCTION_SUPABASE_URL` repository secret — this page previously named a `SUPABASE_URL` secret that no workflow reads and that does not exist, which is a good way to configure half of this and believe it is done.

Verify it rather than assuming: run the **CV parse draft cleanup** workflow manually (`workflow_dispatch`) and confirm a `200`. Until it passes, expired drafts keep their uploaded files and extracted PII past the retention window, and `submission_link_events` / `referral_link_events` grow without bound — nothing else reaps either table. The workflow fails loudly on a missing secret for exactly this reason, but an hourly red cross on a schedule nobody watches is easy to miss for days.
