...
`generate-candidate-profile` and `parse-candidate-cv` both enforce per-user and per-organization hourly caps (20 and 100 profile generations/hour; 10 CV parse dispatches/hour, counting retries), plus a monthly per-organization token ceiling on profile generation, since `ANTHROPIC_API_KEY` is one key for the whole deployment and an unbounded caller can exhaust it for every tenant. The monthly ceiling defaults to 50,000,000 tokens and is overridable per environment via the `AI_PROFILE_MONTHLY_TOKEN_CEILING` Supabase Function secret. `draft-job-requirements` enforces hourly caps only (15 per user, 60 per organization) and no monthly ceiling: it runs on the cheap parse model over a bounded brief and returns at most 40 short rows, so its per-call cost is closer to a CV parse than to a profile generation. Every call still records its token usage on `ai_evaluations`, so if that assumption proves wrong the spend shows up alongside everything else. These are deliberately generous starting points, not tuned limits — watch for `profile_rate_limited`, `org_profile_rate_limited`, `org_monthly_ceiling_reached`, `rate_limited`, and `parse_rate_limited` in the Function logs and tighten once real usage is visible.
