...
- **F4** — dead `.referral-filter` CSS removed. `refer` (staging + production) has no MCP delete tool and CLI deletion is guardrail-blocked, so it was converted to a static `410` tombstone matching the existing `ai-evaluate` pattern instead — verified with a live call on both projects. `ai-evaluate` was already an equivalent tombstone on both projects; no action needed.
- **Staging maintenance cron** — re-established automatically by the production promotion pipeline (`deploy-production-supabase`'s "Schedule in-project maintenance cron" step); production's is confirmed active (`17 * * * *`).
- **P0 fully closed and independently verified on production** (not staging, not a dashboard label): `schedule_maintenance_cron` unauthenticated call now returns `401` (was `200`); 0 anon-executable functions remain (was 9).
- **F5 (P2)** — `supabase/migrations/20260815010000_p2_advisor_hardening.sql`, applied to staging: `normalize_email` now pins `search_path=public` (verified: `proconfig` set, output unchanged); the five deny-all tables now carry an explanatory `comment on table` so the pattern reads as deliberate. `pg_net`'s schema was **not** moved -- Supabase's managed `pg_net` rejects `ALTER EXTENSION ... SET SCHEMA` (non-relocatable), and its callable surface (`net.http_post`) already lives isolated in its own `net` schema, confirmed live, so the advisor's underlying risk is already moot. Leaked-password protection remains a dashboard/Auth-API toggle, not schema DDL.
- **F6 (P2, partial)** — `listCompanies`/`listContacts`/`listJobs` in `src/features/core/repository.ts` now carry an explicit `.limit(1000)` safety cap instead of being truly unbounded. This is a cap, not real pagination: these three feed both list views and picker dropdowns with no dedicated search RPC (unlike candidates' `search_candidates_page`), so building genuine server-side paging is a feature change, not a hardening fix -- left for when live counts approach the cap. Today's query fan-out and the 766 kB bundle are unchanged.
