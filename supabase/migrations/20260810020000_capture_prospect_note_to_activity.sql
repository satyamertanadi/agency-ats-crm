...
-- capture_prospect wrote the extension panel's note into notes + note_links -- and no screen in
-- this application has ever read either table. A consultant who captured a LinkedIn profile with a
-- note lost it silently. That is a data-loss bug, not merely dead weight.
--
-- The note now becomes an activity of type 'note', which ActivityFeed already renders on the
-- candidate record. One store for "things someone wrote about this person", not two. The notes and
-- note_links tables are dropped in 20260810030000_drop_unreachable_schema.sql, once nothing writes
-- to them.

-- activity_type is a check constraint, not an enum, so widening it is a constraint swap. 'note' is
-- deliberately NOT added to log_manual_activity's accepted types: that function's narrowness is
-- what keeps status_change/submission/placement provable as system output, and a hand-written note
-- already has a home in the composer's 'other'.
...
alter table public.activities add constraint activities_activity_type_check
  check (activity_type in ('call','email','whatsapp','meeting','interview','status_change','submission','client_feedback','placement','note','other'));
