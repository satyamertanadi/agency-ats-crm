...
alter table public.candidate_employment
  add column started_on_precision text check (started_on_precision in ('day','month','year')),
  add column ended_on_precision text check (ended_on_precision in ('day','month','year'));
...
create table public.candidate_cv_parses (
  ...
  target_candidate_id uuid references public.candidates(id) on delete cascade,
  matched_candidate_id uuid references public.candidates(id) on delete set null,
  accepted_candidate_id uuid references public.candidates(id) on delete set null,
  original_filename text not null,
  storage_path text not null,
  mime_type text not null,
  size_bytes bigint not null check (size_bytes between 1 and 26214400),
  status text not null default 'uploaded' check (status in ('uploaded','processing','ready','failed','accepted','cancelled','expired')),
  provider text not null default 'anthropic',
  model text,
  prompt_version text not null default 'candidate-cv-v1',
...
create index candidate_cv_parses_owner_status on public.candidate_cv_parses(uploaded_by,status,created_at desc);
create index candidate_cv_parses_expiry on public.candidate_cv_parses(expires_at) where status not in ('accepted','cancelled','expired');

alter table public.candidate_cv_parses enable row level security;

create policy candidate_cv_parses_read_own on public.candidate_cv_parses for select to authenticated
using (
  uploaded_by=auth.uid()
  and public.has_permission(organization_id,'candidates.write')
  and public.has_permission(organization_id,'ai.use')
);

create trigger candidate_cv_parses_touch before update on public.candidate_cv_parses
...
begin
  ...
  candidate_id := coalesce(p_target_candidate_id,parse_row.target_candidate_id);
  if candidate_id is not null and candidate_id is distinct from parse_row.target_candidate_id and candidate_id is distinct from parse_row.matched_candidate_id then
  ...
  if nullif(trim(private_payload->>'email'),'') is not null then
    ...
    where details.organization_id=p_organization_id
      ...
      and (candidate_id is null or details.candidate_id<>candidate_id)
    limit 1;
  end if;
  ...
  if candidate_id is null then
    ...
    if not exists(select 1 from public.candidates where id=candidate_id and organization_id=p_organization_id) then
    ...
    where id=candidate_id and organization_id=p_organization_id;
  end if;
  ...
  insert into public.candidate_private_details(
    ...
    candidate_id,p_organization_id,nullif(trim(private_payload->>'email'),''),nullif(trim(private_payload->>'phone'),''),
  ...
  for item in select value from jsonb_array_elements(coalesce(p_payload->'employment','[]'::jsonb)) loop
    ...
    if not exists(
      select 1 from public.candidate_employment e where e.candidate_id=candidate_id
      ...
      insert into public.candidate_employment(
        ...
        p_organization_id,candidate_id,trim(item->>'company_name'),trim(item->>'title'),nullif(trim(item->>'location'),''),
  ...
  for item in select value from jsonb_array_elements(coalesce(p_payload->'education','[]'::jsonb)) loop
    ...
    if not exists(
      select 1 from public.candidate_education e where e.candidate_id=candidate_id
      ...
      insert into public.candidate_education(
        ...
        p_organization_id,candidate_id,trim(item->>'institution'),nullif(trim(item->>'degree'),''),
  ...
  for item in select value from jsonb_array_elements(coalesce(p_payload->'languages','[]'::jsonb)) loop
    if nullif(trim(item->>'language'),'') is not null then
      ...
      values(p_organization_id,candidate_id,trim(item->>'language'),nullif(trim(item->>'proficiency'),''))
  ...
  for item in select value from jsonb_array_elements(coalesce(p_payload->'skills','[]'::jsonb)) loop
    ...
    values(candidate_id,skill_id,p_organization_id,nullif(trim(item->>'proficiency'),''),nullif(item->>'years_experience','')::numeric)
  ...
  values(p_organization_id,skill_id,candidate_id);

  update public.candidate_cv_parses set status='accepted',accepted_candidate_id=candidate_id,accepted_at=now(),
    extracted_data=null,field_evidence='{}'::jsonb,uncertainties='[]'::jsonb,error_code=null,error_message=null
  where id=p_parse_id;

  insert into public.audit_logs(organization_id,actor_user_id,action,entity_type,entity_id,metadata)
  values(p_organization_id,auth.uid(),'candidate.cv_parse_accepted','candidate',candidate_id,jsonb_build_object('parse_id',p_parse_id));
  return candidate_id;
end $$;
