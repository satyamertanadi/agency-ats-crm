...
Deno.serve(async(request)=>{
  ...
  try{
    ...
    const [{data:organization},{data:job},{data:branding}]=await Promise.all([caller.from('organizations').select('name').eq('id',input.organizationId).single(),caller.from('jobs').select('title,companies(name)').eq('id',input.jobId).single(),caller.from('organization_settings').select('primary_color').eq('organization_id',input.organizationId).maybeSingle()])
    // The one artefact the client actually receives, finally painted in the agency's own colour.
