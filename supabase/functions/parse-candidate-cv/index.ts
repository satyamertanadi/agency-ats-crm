...
async function cancel(request:Request,context:Context,input:ScopedInput,requestID:string){
  ...
  const removed=await context.admin.storage.from(bucket).remove([row.storage_path])
  if(removed.error)throw new FunctionError(503,'cv_storage_delete_failed','The CV file could not be removed. Try cancelling again.')
  const cancelled=await context.admin.from('candidate_cv_parses').update({status:'cancelled',original_filename:`cancelled-${row.id}`,storage_path:`cancelled/${input.organizationId}/${row.id}`,extracted_data:null,field_evidence:{},uncertainties:[],error_code:null,error_message:null}).eq('id',row.id)
...
async function processParse(context:Context,organizationId:string,parseId:string,requestID:string){
  ...
  try{
    ...
    // A truncated response is not malformed JSON, it is an incomplete one -- JSON.parse on it below
    // would throw a raw "Unterminated string" SyntaxError that leaked to the user verbatim. Catch it
    // here, before the parse, with an error the catch block can turn into an honest message.
    ...
    // provider_truncated_response arrives on an HTTP 200 (the request succeeded, the generation did
    // not finish), so it has to be checked before the status-code fallback below or a truncated
    // response would misreport as provider_rejected -- a "configuration problem" it is not.
    const code=error instanceof ProviderError?(error.providerCode==='provider_truncated_response'?'provider_truncated_response':error.status===429?'provider_rate_limited':error.status>=500?'provider_unavailable':'provider_rejected'):'parse_failed'
    const message=code==='provider_rate_limited'?'The CV service is busy. Retry shortly.':code==='provider_unavailable'?'The CV service is temporarily unavailable.':code==='provider_truncated_response'?'This CV was too detailed to finish extracting in one pass. Retry, or attach it without parsing and enter the details manually.':code==='provider_rejected'?'The CV parser rejected this request. This usually means a configuration problem — contact an admin with error code provider_rejected.':'The CV could not be parsed. You can retry or attach it without parsing.'
