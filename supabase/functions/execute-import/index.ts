...
import {industryKey} from '../_shared/industries.ts'
import {resolveOption} from '../_shared/option-sets.ts'
...
/* A migrated CSV is where a free-text sector column does its worst damage: one export can carry "F&B",
 * "Food and Beverage" and "Restaurants" as three industries. Resolving here means the workspace starts
 * filterable instead of needing a thousand hand edits. Unrecognised sectors pass through as typed --
 * the column has no CHECK, and losing a value nobody anticipated is worse than storing it verbatim. */
const industryValue=(row:Row,key:string)=>industryKey(textValue(row,key))||null
// Same treatment for the other retrofitted vocabularies: a column the app now filters by is only as
// good as what the import put in it.
const optionValue=(row:Row,key:string,set:string)=>resolveOption(set,textValue(row,key))||null
