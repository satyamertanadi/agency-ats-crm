export const candidateCvJsonSchema={
  ...
  properties:{
    ...
    employment:{type:'array',items:{type:'object',additionalProperties:false,required:['company_name','title','location','started_on','ended_on','is_current','summary','started_on_precision','ended_on_precision','sort_order'],properties:{company_name:{type:'string'},title:{type:'string'},location:nullableString(),started_on:nullableDate(),ended_on:nullableDate(),is_current:{type:'boolean'},summary:nullableString(),started_on_precision:nullablePrecision(),ended_on_precision:nullablePrecision(),sort_order:{type:'integer',minimum:0}}}},
    education:{type:'array',items:{type:'object',additionalProperties:false,required:['institution','degree','field_of_study','started_on','ended_on','started_on_precision','ended_on_precision','sort_order'],properties:{institution:{type:'string'},degree:nullableString(),field_of_study:nullableString(),started_on:nullableDate(),ended_on:nullableDate(),started_on_precision:nullablePrecision(),ended_on_precision:nullablePrecision(),sort_order:{type:'integer',minimum:0}}}},
...
function nullableNumber(integer=false){return {type:integer?['integer','null']:['number','null'],minimum:0} as const}
