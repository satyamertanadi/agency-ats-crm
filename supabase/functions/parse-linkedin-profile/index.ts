...
Deno.serve(async(request)=>{
  ...
  try{
    ...
    const response=await fetch('https://api.anthropic.com/v1/messages',{
      ...
      body:JSON.stringify({model,max_tokens:6000,system:SYSTEM,messages:[{role:'user',content:[{type:'text',text:`Extract the candidate profile, contact details, employment, education, skills, and languages from this LinkedIn profile text. English and Indonesian are supported.\n\n---\n${clipped}`}]}],output_config:{format:{type:'json_schema',schema:candidateCvJsonSchema}}}),
    })
    ...
    const code=known?error.code:provider?'provider_error':'parse_failed'
    const message=known?error.message:provider?'The AI service is temporarily unavailable. Try again shortly.':'Could not parse this profile.'
