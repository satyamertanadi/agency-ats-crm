...
export function CandidateCvParser({organizationId,userId,targetCandidateId,targetCandidateName,onAccepted,onManual,onCancel,onDirtyChange}:CandidateCvParserProps){
  ...
  return <div className="cv-review stack">
    ...
    <section><h3>Basic details</h3><div className="form-grid"><ReviewField label="Full name" low={low('full_name')}><Input value={draft.full_name} onChange={(event)=>update('full_name',event.target.value)}/></ReviewField><ReviewField label="Current company" low={low('current_company')}><Input value={draft.current_company||''} onChange={(event)=>update('current_company',event.target.value||null)}/></ReviewField><ReviewField label="Current position" low={low('current_position')}><Input value={draft.current_position||''} onChange={(event)=>update('current_position',event.target.value||null)}/></ReviewField><ReviewField label="Location" low={low('location')}><LocationField bare value={draft.location||''} onChange={(value)=>update('location',value||null)}/></ReviewField><Field label="LinkedIn"><Input type="url" value={draft.linkedin_url||''} onChange={(event)=>update('linkedin_url',event.target.value||null)}/></Field><Field label="Portfolio"><Input type="url" value={draft.portfolio_url||''} onChange={(event)=>update('portfolio_url',event.target.value||null)}/></Field><Field label="Source"><Input value={draft.source||''} onChange={(event)=>update('source',event.target.value||null)}/></Field><Field label="Availability"><Input value={draft.availability||''} onChange={(event)=>update('availability',event.target.value||null)}/></Field><Field label="Notice period (days)"><Input type="number" min="0" value={draft.notice_period_days??''} onChange={(event)=>update('notice_period_days',event.target.value?Number(event.target.value):null)}/></Field></div></section>
    <CollapsibleSection title="Private details" forceOpen={sectionNeedsAttention(draft,'private',privateIsEmpty)}>
      <div className="form-grid"><ReviewField label="Email" low={low('private.email')}><Input type="email" value={draft.private.email||''} onChange={(event)=>updatePrivate('email',event.target.value||null)}/></ReviewField><ReviewField label="Phone" low={low('private.phone')}><Input value={draft.private.phone||''} onChange={(event)=>updatePrivate('phone',event.target.value||null)}/></ReviewField><Field label={`Current salary (per ${salaryPeriodLabel})`}><Input type="number" min="0" value={draft.private.current_salary??''} onChange={(event)=>updatePrivate('current_salary',event.target.value?Number(event.target.value):null)}/></Field><Field label={`Expected salary (per ${salaryPeriodLabel})`}><Input type="number" min="0" value={draft.private.expected_salary??''} onChange={(event)=>updatePrivate('expected_salary',event.target.value?Number(event.target.value):null)}/></Field>{/* Explicit empty option rather than defaulting the display to the workspace currency: the draft
        holds null until the reviewer picks, and showing "IDR" over a stored null is exactly the silent
        mismatch the currency picker was introduced to end. */}
      <Field label="Currency"><Select value={draft.private.salary_currency||''} onChange={(event)=>updatePrivate('salary_currency',event.target.value||null)}><option value="">Not recorded</option>{currencyOptions(organization?.base_currency).map((option)=><option key={option.code} value={option.code}>{option.code} — {option.name}</option>)}</Select></Field><Field label="Work authorization"><Input value={draft.private.work_authorization||''} onChange={(event)=>updatePrivate('work_authorization',event.target.value||null)}/></Field></div>
    </CollapsibleSection>
