...
export function AddCandidateToJobModal({open,onClose,candidates:fixedCandidates=[]}:{open:boolean;onClose:()=>void;candidates?:PlacementCandidate[]}){
  const {organization}=useOrganization();const cache=useQueryClient();const [query,setQuery]=useState('');const [selectedCandidateId,setSelectedCandidateId]=useState('');const [jobId,setJobId]=useState('');const [stageId,setStageId]=useState('')
  ...
  const mutation=useMutation({mutationFn:()=>addCandidatesToJob(organization!.id,jobId,candidateIds,stageId||undefined),onSuccess:async()=>{close();await Promise.all([cache.invalidateQueries({queryKey:['pipeline',jobId]}),cache.invalidateQueries({queryKey:['job-health',organization?.id]}),cache.invalidateQueries({queryKey:['candidate-pipelines',organization?.id]}),cache.invalidateQueries({queryKey:['today',organization?.id]})])}})
