...
export function JobCandidatePanel({job,item,stage,stages,currentMemberId,interviews,offers,placement,hasSubmission,action:requestedAction,readOnly,onAction,onClose,onUpdated,onMove,moving,onComposeSubmission,siblingIds,onNavigate,detailLoading=false,detailError}:JobCandidatePanelProps){
  ...
  /* The recommendation and whichever form is open used to be two stacked boxes -- one card telling
   * you what to do next, a second directly below it asking you to do it. They are the same moment in
  ...
  return <Drawer title={item.candidates?.full_name||'Candidate'} description={`${stage.name} · ${job.title}`} eyebrow="Candidate action" open onClose={closePanel} headerActions={pager} onKeyDown={navigation.onKeyDown}>
    ...
    <section className={`next-action-card${showForm?' next-action-card-form':''}`}>
      {showForm?<div className="next-action-form-fields">
        {/* 'check_feedback'/'check_offer'/'outcome'/'retry_cancel' have no form of their own -- the
          * feedback and offer status live on the lifecycle cards below, so those four leave this box
          * showing the recommendation still (see showForm above). */}
        {action==='interview'&&<><h3>{rescheduling?'Reschedule interview':'Schedule interview'}</h3><div className="form-grid">{/* The column existed and was hardcoded to 'client_interview' at the only call site, so a phone screen and a final panel were indistinguishable in the record and in any report over it. */}<Field label="Interview type"><OptionSelect label="Interview type" placeholder="Client interview" options={interviewType.options(interviewKind)} value={interviewType.key(interviewKind)} onChange={setInterviewKind}/></Field><Field label="Starts" hint={dateTimeHint(startsAt,organization?.timezone)}><Input type="datetime-local" value={startsAt} onChange={(event)=>setStartsAt(event.target.value)}/></Field><Field label="Ends" hint={dateTimeHint(endsAt,organization?.timezone)}><Input type="datetime-local" value={endsAt} onChange={(event)=>setEndsAt(event.target.value)}/></Field></div><Field label="Location or call details"><Input value={location} onChange={(event)=>setLocation(event.target.value)}/></Field><Field label="Attendee emails"><Input value={attendeeEmails} onChange={(event)=>setAttendeeEmails(event.target.value)}/></Field><label className="check-row"><input type="checkbox" checked={createMeet} disabled={!calendarConnected} onChange={(event)=>setCreateMeet(event.target.checked)}/><span>Create a Google Meet link and synchronize my Calendar</span></label>{!calendarConnected&&<p className="muted">Calendar is not connected. The interview will still be saved in the ATS.</p>}{interview.error&&<p className="form-error" role="alert">{interview.error.message}</p>}<ActionButtons onCancel={()=>{setRescheduling(null);onAction(null)}} label={rescheduling?'Move interview':'Schedule interview'} loading={interview.isPending} disabled={!startsAt||!endsAt} onSave={()=>interview.mutate()}/></>}
        {action==='offer'&&<><h3>Record offer</h3><Field label="Salary"><Input type="number" min="0" value={salary} onChange={(event)=>setSalary(event.target.value)}/></Field><Field label="Currency"><Input value={job.currency||organization!.base_currency} disabled/></Field><Field label="Proposed start"><Input type="date" value={startDate} onChange={(event)=>setStartDate(event.target.value)}/></Field>{offer.error&&<p className="form-error" role="alert">{offer.error.message}</p>}<ActionButtons onCancel={()=>onAction(null)} label="Record offer" loading={offer.isPending} disabled={!salary} onSave={()=>offer.mutate()}/></>}
        {action==='placement'&&<><h3>Create placement</h3>{acceptedOffer?<><p className="action-fact action-fact-good"><CheckCircle2 size={15}/>Accepted offer · {formatMoney(acceptedOffer.salary,acceptedOffer.currency)}</p>{expectedFee!=null&&expectedSource
          ?<div className="action-fact action-fact-info"><span>Expected fee <strong>{formatMoney(expectedFee,jobHealth?.currency||job.currency||undefined)}</strong> · {jobHealth?.fee_source}</span><Button size="sm" variant="secondary" type="button" onClick={()=>{setFee(String(expectedFee));setFeeSource(expectedSource)}}>Use this fee</Button></div>
          :<p className="action-fact action-fact-warn"><TriangleAlert size={15}/>This job has no agreed fee terms, so the placement fee will be recorded as manually priced.</p>}<Field label="Placement fee"><Input type="number" min="0" value={fee} onChange={(event)=>{setFee(event.target.value);setFeeSource('manual')}}/></Field><p className="muted">Recorded as <strong>{feeSource==='manual'?'manually priced':feeSource==='job_override'?'the job fee override':'the account fee agreement'}</strong>. The 90-day guarantee default will be applied.</p>{place.error&&<p className="form-error" role="alert">{place.error.message}</p>}<ActionButtons onCancel={()=>onAction(null)} label="Create placement" loading={place.isPending} disabled={!fee} onSave={()=>place.mutate()}/></>:<p className="action-fact action-fact-warn">An accepted offer is required before creating a placement.</p>}</>}
        ...
        {action==='move'&&<>
          ...
          <ActionButtons onCancel={()=>onAction(null)} label="Move candidate" loading={moving} disabled={moveStage===item.current_stage_id} onSave={submitMove}/>
        </>}
      </div>:<>
