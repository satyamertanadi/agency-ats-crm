...
import {filterJobHealth,phaseSegments,type JobHealthFilter} from './jobHealth'

const filters:Array<[JobHealthFilter,string]>=[['all','All active'],['unowned','Unowned'],['empty','No candidates'],['stale','Stale'],['interview','Interview'],['offer','Offer'],['high_value','High value'],['urgent','Urgent']]

/* The list used to hardcode its base query to open/draft/on_hold, so a filled or closed job had no
 * URL that could show it -- not "hidden behind a filter" but genuinely unreachable from this page.
 * A separate axis from the health filters above: this decides WHICH jobs are in the set at all,
 * health then refines within it. Defaults to 'open' so the day-to-day view is unchanged. */
type JobStatusFilter='open'|'filled'|'closed'|'all'
const statusFilters:Array<[JobStatusFilter,string]>=[['open','Open'],['filled','Filled'],['closed','Closed'],['all','All']]
const matchesStatusFilter=(status:string,filter:JobStatusFilter)=>{
  if(filter==='all')return true
  if(filter==='open')return status==='draft'||status==='open'||status==='on_hold'
...
export function JobsPage(){
  ...
  const search=params.get('q')||'';const filter=(params.get('health') as JobHealthFilter)||'all';const statusFilter=(params.get('status') as JobStatusFilter)||'open'
  ...
  /* create_job_with_pipeline (company_id/title/owner) is the whole creation act -- it inserts the job
   * and its default pipeline in one RPC, so once it returns an id the job unambiguously exists.
   * Location/priority/description/salary/currency used to be a second, blocking updateJob call
   * inside the same mutation: if that second call failed for any reason, onError fired "The job was
   * not created" over a job that very much had been. Now those are optional follow-ups fired AFTER
   * the consultant has already been navigated to the real job, so a failure there can be reported for
   * what it actually is -- some details did not save -- rather than as a creation that never happened. */
  const mutation=useMutation({
    mutationFn:()=>createJob(organization!.id,{company_id:companyId,title,owner_member_id:owner||undefined}),
    onSuccess:async(id)=>{
      ...
      toast.success(`${createdTitle} is open and ready for candidates.`)
      const extra:Record<string,unknown>={}
      if(location)extra.location=location
      if(priority!=='normal')extra.priority=priority
      if(description)extra.description=description
      if(salaryMin)extra.salary_min=Number(salaryMin)
      if(salaryMax)extra.salary_max=Number(salaryMax)
      if(currency)extra.currency=currency
      if(Object.keys(extra).length===0)return
      try{await updateJob(organization!.id,id,extra);await cache.invalidateQueries({queryKey:['job-health',organization?.id]})}
      catch(error){toast.error(error,`${createdTitle} was created, but its extra details were not saved -- edit the job to add them.`)}
    },
  ...
  const needle=search.trim().toLowerCase();const visible=filterJobHealth((jobs.data||[]).filter((job)=>matchesStatusFilter(job.status,statusFilter)),filter).filter((job)=>!needle||[job.title,job.company_name,job.location,job.owner_name].some((value)=>value?.toLowerCase().includes(needle))).sort((a,b)=>(a.priority==='urgent'?0:a.priority==='high'?1:2)-(b.priority==='urgent'?0:b.priority==='high'?1:2)||b.days_open-a.days_open)
  /* next_action used to link every row to the exact same bare job URL no matter what it said --
   * "Assign an owner" and "Add candidates" promised a specific act and then made you go find it
   * yourself once there. These three map to a surface JobWorkspacePage actually knows how to open on
   * arrival; anything else still lands on the board, which honestly is the acting surface for it. */
  const nextActionHref=(job:{id:string;next_action:string|null})=>{
    const base=`/app/${organization?.slug}/jobs/${job.id}`
    if(job.next_action==='Assign an owner')return `${base}?editJob=1`
    if(job.next_action==='Add candidates')return `${base}?addCandidates=1`
    if(job.next_action==='Log first activity')return `${base}?view=activity`
    return base
  }
