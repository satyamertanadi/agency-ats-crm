...
export function JobsPage(){
  ...
  const search=params.get('q')||'';const filter=(params.get('health') as JobHealthFilter)||'all'
  ...
  const mutation=useMutation({mutationFn:async()=>{const id=await createJob(organization!.id,{company_id:companyId,title,owner_member_id:owner||undefined});await updateJob(organization!.id,id,{location:location||null,priority});return id},onSuccess:async(id)=>{setOpen(false);setTitle('');setLocation('');await Promise.all([cache.invalidateQueries({queryKey:['jobs',organization?.id]}),cache.invalidateQueries({queryKey:['job-health',organization?.id]})]);navigate(`/app/${organization!.slug}/jobs/${id}`);toast.success(`${title} is open and ready for candidates.`)},onError:(error)=>toast.error(error,'The job was not created.')})
  ...
  const needle=search.trim().toLowerCase();const visible=filterJobHealth((jobs.data||[]).filter((job)=>['open','draft','on_hold'].includes(job.status)),filter).filter((job)=>!needle||[job.title,job.company_name,job.location,job.owner_name].some((value)=>value?.toLowerCase().includes(needle))).sort((a,b)=>(a.priority==='urgent'?0:a.priority==='high'?1:2)-(b.priority==='urgent'?0:b.priority==='high'?1:2)||b.days_open-a.days_open)
