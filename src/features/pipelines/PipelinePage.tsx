...
import { addCandidateToJob,getPipeline,listCandidates,listJobs } from '../core/repository'
...
export function PipelinePage(){
  const {jobId}=useParams();const {organization}=useOrganization();const {user}=useAuth();const cache=useQueryClient();const toast=useToast();const [open,setOpen]=useState(false);const [candidateId,setCandidateId]=useState('')
  ...
  const move=useStageMove(jobId||'')
  const add=useMutation({mutationFn:()=>addCandidateToJob(organization!.id,user!.id,job!,candidateId),onSuccess:async()=>{const name=candidates.data?.find((candidate)=>candidate.id===candidateId)?.full_name;setOpen(false);setCandidateId('');await cache.invalidateQueries({queryKey:['pipeline',jobId]});toast.success(`${name||'Candidate'} was added to ${job!.title}.`)},onError:(error)=>toast.error(error,'No candidate was added.')})
  const onDragEnd=(event:DragEndEvent)=>{const itemId=String(event.active.id);const stageId=event.over?.id?String(event.over.id):'';const current=pipeline.data?.items.find((item)=>item.id===itemId);if(stageId&&current?.current_stage_id!==stageId&&pipeline.data?.stages.some((stage)=>stage.id===stageId))move.mutate({itemId,stageId,name:current?.candidates?.full_name,label:pipeline.data.stages.find((stage)=>stage.id===stageId)?.name||'the next stage'})}
  if(jobs.isLoading||pipeline.isLoading)return <Panel><BoardSkeleton label="Loading vacancy pipeline…"/></Panel>;if(jobs.error||pipeline.error||!job)return <ErrorState error={jobs.error||pipeline.error||new Error('Vacancy not found')}/>
  return <Page title={job.title} eyebrow={job.companies?.name||'Vacancy pipeline'} description="Drag candidates or use the stage selector. Every movement is recorded in immutable stage history." actions={<Button leadingIcon={<Plus size={15}/>} onClick={()=>setOpen(true)}>Add candidate</Button>}><Panel><DndContext sensors={sensors} onDragEnd={onDragEnd}><div className="kanban">{pipeline.data?.stages.map((stage)=>{const items=pipeline.data.items.filter((item)=>item.current_stage_id===stage.id);return <StageColumn stage={stage} count={items.length} key={stage.id}>{items.map((item)=><CandidateCard item={item} stages={pipeline.data.stages} key={item.id} onMove={(stageId)=>move.mutate({itemId:item.id,stageId,name:item.candidates?.full_name,label:pipeline.data.stages.find((option)=>option.id===stageId)?.name||'the next stage'})}/>)}</StageColumn>})}</div></DndContext></Panel><Modal title="Add candidate to vacancy" open={open} onClose={()=>setOpen(false)}><div className="stack"><Field label="Candidate"><Select value={candidateId} onChange={(event)=>setCandidateId(event.target.value)}><option value="">Select candidate</option>{candidates.data?.filter((candidate)=>!pipeline.data?.items.some((item)=>item.candidate_id===candidate.id)).map((candidate)=><option value={candidate.id} key={candidate.id}>{candidate.full_name}</option>)}</Select></Field>{add.error&&<p className="form-error" role="alert">{add.error.message}</p>}<div className="form-actions"><Button variant="quiet" onClick={()=>setOpen(false)}>Cancel</Button><Button disabled={!candidateId||add.isPending} onClick={()=>add.mutate()}>Add to first stage</Button></div></div></Modal></Page>
}
