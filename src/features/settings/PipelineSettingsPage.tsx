...
export function PipelineSettingsPage(){
  ...
  const update=useMutation({mutationFn:({id,phaseKey}:{id:string;phaseKey:PipelinePhaseKey|null})=>updatePipelineStagePhase(organization!.id,id,phaseKey),onSuccess:async()=>{toast.success('Phase mapping updated.','Existing stage history is never rewritten.');await cache.invalidateQueries({queryKey:['pipeline-settings',organization?.id]})},onError:(error)=>toast.error(error,'The mapping is unchanged.')})
