...
export async function listContacts(organizationId:string){const {data,error}=await supabase.from('contacts').select('id,organization_id,company_id,full_name,position,email,phone,contact_status,next_follow_up_at,companies(id,name)').eq('organization_id',organizationId).is('deleted_at',null).order('updated_at',{ascending:false}).limit(LIST_SAFETY_LIMIT);if(error)fail(error,'Could not load contacts');return rows(data,contactSchema,'Contact rows did not match the expected shape') as Contact[]}
