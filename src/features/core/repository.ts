...
export async function listOffers(organizationId:string){const {data,error}=await supabase.from('offers').select('id,job_candidate_id,salary,currency,offered_at,start_date,status,notes,job_candidates(candidates(full_name),jobs(title))').eq('organization_id',organizationId).order('offered_at',{ascending:false});if(error)fail(error,'Could not load offers');return (data??[]) as unknown as Offer[]}
...
export async function listSubmissionPackages(organizationId:string){const {data,error}=await supabase.from('submission_packages').select('id,title,message,status,created_at,jobs(title,companies(name)),candidate_submissions(id,status),public_submission_links(id,recipient_email,expires_at,revoked_at,last_accessed_at)').eq('organization_id',organizationId).order('created_at',{ascending:false});if(error)fail(error,'Could not load submissions');return data??[]}
