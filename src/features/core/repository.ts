...
import type { Activity, Candidate, Company, Contact, Interview, Job, JobCandidate, Offer, PipelineStage, Placement, PublicReferral, PublicReview, Referral, ReferralLink, ReferralStatus, Task } from '../../shared/types/domain'
...
export async function listCandidates(organizationId:string, query='') {
  ...
  const {data,error}=await request;if(error)fail(error,'Could not load candidates');return (data??[]) as unknown as Candidate[]
}

export async function listCandidatesPage(organizationId:string,query='',page=0,pageSize=50){
  let request=supabase.from('candidates').select('id,organization_id,full_name,current_company,current_position,location,linkedin_url,status,source,availability,owner_member_id,created_at,candidate_private_details(email,phone,current_salary,expected_salary,salary_currency,consent_status)',{count:'exact'}).eq('organization_id',organizationId).is('deleted_at',null).order('updated_at',{ascending:false}).range(page*pageSize,page*pageSize+pageSize-1)
  if(query.trim())request=request.or(`full_name.ilike.%${query.trim()}%,current_company.ilike.%${query.trim()}%,current_position.ilike.%${query.trim()}%`)
  const {data,error,count}=await request;if(error)fail(error,'Could not load candidates');return {rows:(data??[]) as unknown as Candidate[],count:count??0}
}
...
export async function listCompanies(organizationId:string){const {data,error}=await supabase.from('companies').select('*').eq('organization_id',organizationId).is('deleted_at',null).order('updated_at',{ascending:false});if(error)fail(error,'Could not load companies');return (data??[]) as unknown as Company[]}
export async function createCompany(organizationId:string,userId:string,input:{name:string;industry?:string;location?:string;website?:string;account_status:string}){const {data,error}=await supabase.from('companies').insert({...input,organization_id:organizationId,created_by:userId}).select('id').single();if(error)fail(error,'Could not create company');return data.id as string}

export async function listContacts(organizationId:string){const {data,error}=await supabase.from('contacts').select('id,organization_id,company_id,full_name,position,email,phone,contact_status,next_follow_up_at,companies(id,name)').eq('organization_id',organizationId).is('deleted_at',null).order('updated_at',{ascending:false});if(error)fail(error,'Could not load contacts');return (data??[]) as unknown as Contact[]}
...
export async function listJobs(organizationId:string){const {data,error}=await supabase.from('jobs').select('id,organization_id,company_id,pipeline_id,title,location,priority,status,currency,placement_fee_percentage,owner_member_id,opened_at,updated_at,companies(id,name)').eq('organization_id',organizationId).is('deleted_at',null).order('updated_at',{ascending:false});if(error)fail(error,'Could not load jobs');return (data??[]) as unknown as Job[]}
...
export async function listTasks(organizationId:string){const {data,error}=await supabase.from('tasks').select('*,organization_members:owner_member_id(profiles:user_id(full_name,email)),task_links(candidate_id,company_id,contact_id,job_id,candidates(full_name),companies(name),contacts(full_name),jobs(title))').eq('organization_id',organizationId).is('deleted_at',null).order('due_at',{ascending:true,nullsFirst:false});if(error)fail(error,'Could not load tasks');return (data??[]) as unknown as Task[]}
...
export async function listOffers(organizationId:string){const {data,error}=await supabase.from('offers').select('id,job_candidate_id,salary,currency,offered_at,start_date,status,notes,job_candidates(candidate_id,candidates(id,full_name),jobs(id,title,owner_member_id))').eq('organization_id',organizationId).order('offered_at',{ascending:false});if(error)fail(error,'Could not load offers');return (data??[]) as unknown as Offer[]}
...
export async function convertOfferToPlacement(offerId:string,fee:number,guaranteeDays=90){const {data,error}=await supabase.rpc('create_placement_from_offer',{p_offer_id:offerId,p_fee:fee,p_guarantee_days:guaranteeDays});if(error)fail(error,'Could not convert accepted offer to placement');return data as string}
...
export async function resolveReview(token:string){const {data,error}=await supabase.functions.invoke('public-review',{body:{action:'resolve',token},timeout:10_000});if(error)fail(error,'This review link is unavailable.');if((data as {error?:{message?:string}})?.error)throw new AppError((data as {error:{message?:string}}).error.message||'This review link is unavailable.','link_unavailable',data);return data as unknown as PublicReview}
...
// Authenticated side.
export async function createReferralLink(organizationId:string,input:{label?:string;memberId?:string}={}){const {data,error}=await supabase.rpc('create_referral_link',{p_organization_id:organizationId,p_label:input.label||undefined,p_member_id:input.memberId||undefined});if(error)fail(error,'Could not create referral link');return data as unknown as {link_id:string;token:string;expires_at:string|null}}
export async function listReferralLinks(organizationId:string){const {data,error}=await supabase.from('referral_links').select('id,label,token_prefix,member_id,expires_at,revoked_at,created_at').eq('organization_id',organizationId).is('revoked_at',null).order('created_at',{ascending:false});if(error)fail(error,'Could not load referral links');return (data??[]) as unknown as ReferralLink[]}
...
export async function acceptReferral(organizationId:string,referralId:string){const {data,error}=await supabase.rpc('accept_referral',{p_organization_id:organizationId,p_referral_id:referralId});if(error)fail(error,'Could not accept this referral');return data as unknown as {candidate_id:string;deduped:boolean}}
