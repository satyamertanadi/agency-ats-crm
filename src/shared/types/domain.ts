...
/* The four interview capabilities are UX only, like every other field here: each one is independently
 * enforced by RLS in 20260830000000_interview_intelligence_core.sql. canViewOwnInterviewQuality and
 * canReviewTeamInterviewQuality are separate on purpose -- a consultant seeing findings about their
 * own interviews is a different grant from seeing a colleague's, and collapsing them into one flag is
 * how the second quietly becomes implied by ordinary candidate access. */
export interface WorkspaceCapabilities {roleKeys:string[];canWriteCandidates:boolean;canWriteClients:boolean;canWriteJobs:boolean;canMovePipeline:boolean;canSubmit:boolean;canManageInterviews:boolean;canManageOffers:boolean;canManagePlacements:boolean;canManageCommercialTerms:boolean;canViewTeamReports:boolean;canManageFinance:boolean;canImport:boolean;canManageOrganization:boolean;canManageWorkspace:boolean;canManageTemplates:boolean;canViewAdmin:boolean;readOnly:boolean;canUseInterviewIntelligence:boolean;canViewOwnInterviewQuality:boolean;canReviewTeamInterviewQuality:boolean;canConfigureInterviewIntelligence:boolean}
...
export type CalendarConnection={id:string;organization_id:string;member_id:string;google_email:string;calendar_id:string;status:CalendarConnectionStatus;connected_at:string;last_synced_at:string|null;last_error:string|null;scopes:string[]}
