...
import { BarChart3,BriefcaseBusiness,Building2,CalendarRange,CheckSquare,ChevronDown,ContactRound,FileUp,LayoutDashboard,LogOut,Menu,PanelLeftClose,Plus,Search,Send,Settings,SlidersHorizontal,UserRoundSearch,WalletCards,X } from 'lucide-react'
...
export function AppShell() {
  ...
  return <div className={`app-layout ${collapsed?'sidebar-collapsed':''}`} style={style}>
    <aside className={`sidebar ${mobileOpen?'sidebar-open':''}`}>
      <div className="brand"><span className="brand-mark">{logoUrl?<img src={logoUrl} alt=""/>:initials}</span><div><strong>{organization?.name||env.productName}</strong><small>{simplified?'Recruitment workspace':'Agency operations'}</small></div><button className="icon-button mobile-only" onClick={()=>setMobileOpen(false)} aria-label="Close navigation"><X size={18}/></button></div>
