...
export function OrganizationProvider({children}:{children:ReactNode}){
  ...
  const query=useQuery({queryKey:['memberships',user?.id],enabled:Boolean(user),queryFn:async()=>{
    ...
    return (data??[]).map((membership)=>{
      ...
      return {...membership,organizations:{id:raw.id,name:raw.name,slug:raw.slug,base_currency:raw.base_currency,salary_period:raw.salary_period,timezone:raw.timezone,seat_limit:raw.seat_limit,pilot_status:raw.pilot_status,primary_color:settings?.primary_color,logo_path:settings?.logo_path,logo_url:logoUrl,profile_enabled:settings?.settings?.profile_v1===true,consultant_first_enabled:settings?.settings?.consultant_first_v1!==false}} as unknown as Membership
    })
  }})
