...
export function OrganizationProvider({children}:{children:ReactNode}){
  ...
  const query=useQuery({queryKey:['memberships',user?.id],enabled:Boolean(user),queryFn:async()=>{
    const {data,error}=await supabase.from('organization_members').select('id, organization_id, user_id, status, organizations!inner(id,name,slug,base_currency,timezone,seat_limit,organization_settings(primary_color,logo_path))').eq('status','active')
    ...
    return (data??[]).map((membership)=>{
      ...
      return {...membership,organizations:{id:raw.id,name:raw.name,slug:raw.slug,base_currency:raw.base_currency,timezone:raw.timezone,seat_limit:raw.seat_limit,primary_color:settings?.primary_color,logo_path:settings?.logo_path,logo_url:logoUrl}} as unknown as Membership
    })
  }})
