...
function Protected(){const {user,loading,error:authError}=useAuth();const {membership,organization,loading:orgLoading,error:orgError,refresh}=useOrganization();const location=useLocation();if(authError||orgError)return <ErrorState error={authError||orgError} retry={orgError?()=>void refresh():undefined}/>;if(loading||orgLoading)return <LoadingState label="Opening workspace…"/>;if(!user)return <Navigate to={`/login?next=${encodeURIComponent(location.pathname+location.search)}`} replace/>;if(!membership||!organization)return <Navigate to={env.allowSelfServiceOnboarding?'/onboarding':'/access-pending'} replace/>;return <AppShell/>}
