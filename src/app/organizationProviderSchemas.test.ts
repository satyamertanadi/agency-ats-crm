...
describe('membershipRowSchema',()=>{
  it('accepts a membership row with a singular organization_settings embed (its real 1:1 shape)',()=>{
    const row={
      ...
      organizations:{id:'o1',name:'Acme Agency',slug:'acme',base_currency:'IDR',salary_period:'monthly',timezone:'Asia/Makassar',seat_limit:10,pilot_status:'active',
  ...
  it('accepts a null organization_settings embed (no settings row yet)',()=>{
    const row={
      id:'m1',organization_id:'o1',user_id:'u1',status:'active',
      organizations:{id:'o1',name:'Acme Agency',slug:'acme',base_currency:'IDR',salary_period:'monthly',timezone:'Asia/Makassar',seat_limit:10,pilot_status:'active',organization_settings:null},
    }
    expect(membershipRowSchema.safeParse(row).success).toBe(true)
  })
})
