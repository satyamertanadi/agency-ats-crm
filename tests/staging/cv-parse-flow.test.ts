...
it('generates persisted Indonesian evidence and finalizes only with both private formats',async(ctx)=>{
  ...
  // A PDF must not pass as the client document: the mandatory client template is a Word file, so the
  // RPC accepts a DOCX mime only. This is what stops the old dual-format path silently returning.
  const wrongFormat=await user.rpc('finalize_candidate_profile',{p_organization_id:organizationId,p_profile_version_id:payload.profileVersionId,p_reviewed_content:payload.draft,p_anonymized:true,p_docx_document_id:pdfId,p_edited_field_count:0});expect(wrongFormat.error).not.toBeNull()
  ...
  const finalized=await user.rpc('finalize_candidate_profile',{p_organization_id:organizationId,p_profile_version_id:payload.profileVersionId,p_reviewed_content:payload.draft,p_anonymized:true,p_docx_document_id:docxId,p_edited_field_count:0})
  expect(finalized.error,finalized.error?.message).toBeNull()
  // Re-finalizing with the same document is idempotent; a different one is refused.
  const repeat=await user.rpc('finalize_candidate_profile',{p_organization_id:organizationId,p_profile_version_id:payload.profileVersionId,p_reviewed_content:payload.draft,p_anonymized:true,p_docx_document_id:docxId,p_edited_field_count:0});expect(repeat.error,repeat.error?.message).toBeNull()
  const immutable=await user.rpc('finalize_candidate_profile',{p_organization_id:organizationId,p_profile_version_id:payload.profileVersionId,p_reviewed_content:{...payload.draft,candidate_summary:['Tampered after finalization.']},p_anonymized:false,p_docx_document_id:pdfId,p_edited_field_count:1});expect(immutable.error).not.toBeNull()
  ...
  expect(finalRow.error,finalRow.error?.message).toBeNull();expect(finalRow.data?.status).toBe('finalized');expect(finalRow.data?.exported_formats).toEqual(['docx'])
